...
const db = mysql.createConnection({
    ...
    database: "projet_db", // Crée cette base de données dans MySQL
});
...
app.post("/inscription", (req, res) => {
    const { ID, nom, prenom, tel, naissance, email } = req.body;

    // Requête SQL pour insérer les données dans la base de données
    // console.log(nom, email, motdepasse);
    
    const query =
        "INSERT INTO people (ID, nom, prenom, tel, naissance, email) VALUES (?, ?, ?, ?, ?, ?);";

    db.query(query, [ID, nom, prenom, tel, naissance, email], (err, result) => {
        if (err) {
